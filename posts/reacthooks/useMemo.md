...
useMemo 是 React 提供的一个内置 Hook，它的核心设计目标是**缓存计算昂贵的函数结果**，避免在每次组件重新渲染时都重复执行高开销的计算逻辑。从官方定义来看，useMemo 接收一个“创建函数”和一个依赖数组，仅当依赖数组中的某个依赖项发生变化时，才会重新执行创建函数并返回新的计算结果；若依赖未变化，则直接返回上一次缓存的结果。

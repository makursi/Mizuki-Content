...
# 异步组件的官方定义
在React中，异步组件（Async Components）是指**不会在页面初始渲染时同步加载**，而是在需要时（如组件即将被渲染、满足特定业务条件等）通过异步方式动态加载的组件。React官方通过`React.lazy`配合`Suspense`实现异步组件加载能力，其核心依托ES模块的动态导入（`import()`）特性，本质是将组件的加载过程从同步执行流中剥离，实现代码分割（Code Splitting），从而减小应用初始打包体积，提升首屏加载性能。

...
## 组件的官方定义
在 React 中，组件是可复用的、独立的代码单元，用于封装 UI 中的一部分，可将 UI 拆分为独立且可复用的片段，并对每个片段进行独立思考。从技术角度来讲，React 组件本质上是接受入参（props）并返回 React 元素（描述屏幕上应显示内容）的 JavaScript 函数（基于 TypeScript 开发时则是类型化的函数/类）。
